...
def check(name, mid, expected):
    m = client.wait_model(mid, interval=3, timeout=600, rtx=RTX)
    actual = m.params or {}
    ok, issues = True, []
...
run("gen_360 a_pose=True",  lambda: check("gen_360 a_pose=True",
    client.gen_360(str(ASSETS/"main_view.png"), "VISVISE-MultiView-V1.0.0", name="opt_360a", enable_a_pose=True, rtx=RTX),
    {"image_gen_360_params.enable_a_pose": True}))
run("gen_360 a_pose=False", lambda: check("gen_360 a_pose=False",
    client.gen_360(str(ASSETS/"main_view.png"), "VISVISE-MultiView-V1.0.0", name="opt_360b", enable_a_pose=False, rtx=RTX),
...
run("high face_num=100000", lambda: check("high face_num=100000",
    client.gen_high_model(MV["main"], "Tripo-v3.1-ultra", face_type=1, name="opt_hm_a", face_num=100000, rtx=RTX),
...
run("high face_type=2 glb", lambda: check("high face_type=2 glb",
    client.gen_high_model(MV["main"], "Tripo-v3.1-ultra", face_type=2, output_model_format="glb", name="opt_hm_b", rtx=RTX),
...
run("low face_type=1 back", lambda: check("low face_type=1 back",
    client.gen_low_model(MV["main"], "Tripo-v1.0-快速生成", face_type=1, back_view=MV["back"], name="opt_lm_a", rtx=RTX),
    {"image_gen_model_params.face_type": 1}))
run("low face_type=2 fbx",  lambda: check("low face_type=2 fbx",
    client.gen_low_model(MV["main"], "Tripo-v1.0-快速生成", face_type=2, output_model_format="fbx", name="opt_lm_b", rtx=RTX),
